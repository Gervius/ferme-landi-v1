...
import { customerPaymentsIndex, customerPaymentsStore } from '@/routes';
...
export default function CreateCustomerPayment({ customers }: Props) {
    ...
    const breadcrumbs = [
        { title: 'Ventes', href: '#' },
        { title: 'Paiements', href: customerPaymentsIndex.url() },
    ...
    return (
        <div className="p-6 max-w-4xl mx-auto space-y-6">
            ...
            <div className="flex justify-between items-center text-sm">
                ...
                <Link href={customerPaymentsIndex.url()} className="text-muted-foreground hover:text-foreground flex items-center gap-1 transition">
            ...
            <form onSubmit={handleSubmit} className="bg-card rounded-xl border border-border shadow-lg overflow-hidden">
                ...
                <div className="p-6 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-1.5">
                            <label className="text-xs font-bold uppercase text-muted-foreground tracking-tighter">Référence de transaction</label>
                            <input
                                type="text"
                                value={data.reference}
                                onChange={e => setData('reference', e.target.value)}
                                className="w-full bg-background border border-border rounded-lg px-3 py-2.5 font-mono font-bold"
                                placeholder="N° Pièce / Réf. Transaction"
                            />
